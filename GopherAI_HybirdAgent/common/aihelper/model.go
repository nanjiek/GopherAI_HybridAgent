...
func NewOpenAIModel(ctx context.Context) (*OpenAIModel, error) {
	...
	key := providerSetting("OPENAI_API_KEY", conf.OpenAIConfig.APIKey)
	modelName := providerSetting("OPENAI_MODEL_NAME", conf.OpenAIConfig.ModelName)
	baseURL := providerSetting("OPENAI_BASE_URL", conf.OpenAIConfig.BaseURL)
	if key == "" || modelName == "" || baseURL == "" {
		return nil, fmt.Errorf("openai config is incomplete")
	}
...
func NewKimiModel(ctx context.Context) (*KimiModel, error) {
	conf := config.GetConfig()
	key := providerSetting("KIMI_API_KEY", conf.KimiConfig.APIKey)
	modelName := providerSetting("KIMI_MODEL_NAME", conf.KimiConfig.ModelName)
	baseURL := providerSetting("KIMI_BASE_URL", conf.KimiConfig.BaseURL)
	if key == "" || modelName == "" || baseURL == "" {
		return nil, fmt.Errorf("kimi config is incomplete")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("create kimi model failed: %v", err)
	}
	return &KimiModel{llm: llm}, nil
}

func (k *KimiModel) GenerateResponse(ctx context.Context, messages []*schema.Message) (*schema.Message, error) {
	resp, err := k.llm.Generate(ctx, messages)
	if err != nil {
		return nil, fmt.Errorf("kimi generate failed: %v", err)
	}
	return resp, nil
}

func (k *KimiModel) StreamResponse(ctx context.Context, messages []*schema.Message, cb StreamCallback) (string, error) {
	stream, err := k.llm.Stream(ctx, messages)
	if err != nil {
		return "", fmt.Errorf("kimi stream failed: %v", err)
	}
	defer stream.Close()
	...
	for {
		...
		if recvErr != nil {
			return "", fmt.Errorf("kimi stream recv failed: %v", recvErr)
		}
		if len(msg.Content) > 0 {
...
func (k *KimiModel) GetModelType() string { return "5" }

func providerSetting(envKey, cfgValue string) string {
...
func NewMCPModel(ctx context.Context, username string) (*MCPModel, error) {
	...
	mcpBaseURL := strings.TrimSpace(conf.MCPConfig.BaseURL)
